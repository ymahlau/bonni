...
def optimize_bonni(
    ...
    if xs is not None or ys is not None or gs is not None:
        assert xs is not None and ys is not None and gs is not None, (
            "if previous samples given, all xs, ys and gs need to be provided"
        )
        assert xs.ndim == 2 and xs.shape[1] == num_actions, (
            f"invalid shape for xs: {xs.shape}"
        )
        num_prev_samples = xs.shape[0]
        assert ys.ndim == 1 and ys.shape[0] == num_prev_samples, (
            f"invalid shape for ys: {ys.shape}"
        )
        assert gs.ndim == 2 and gs.shape == xs.shape, (
            f"invalid shape for gs: {gs.shape}"
        )
    ...
    if num_random_samples is not None:
        assert num_random_samples >= 0, (
            f"invalid random sample count: {num_random_samples}"
        )
    ...
    assert x_list, (
        f"Need to either specify random sample count or provide previous samples, got {num_random_samples=} and {num_prev_samples=}"
    )
