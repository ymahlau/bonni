...
class FunctionWrapper:
    ...
    def __call__(
        ...
        assert np.all(x > self.action_bounds[:, 0]) and np.all(
            x < self.action_bounds[:, 1]
        ), f"Invalid function input: {x}"
        ...
        assert len(fn_output) == 2, (
            f"If function output is sequence, it needs to have length 2, but got {fn_output}"
        )
        y_out, g_out = fn_output[0], fn_output[1]
        assert isinstance(y_out, np.ndarray | float), (
            f"Invalid function output at pos 0: {y_out}"
        )
        assert isinstance(g_out, np.ndarray | float), (
            f"Invalid function output at pos 1: {g_out}"
        )
        ...
        assert g_flat.size == self.action_bounds.shape[0], (
            f"Invalid fn output at pos 1: {g_out}"
        )
