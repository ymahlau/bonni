...
def visualize_bo_1d(
    ...
    ax1.fill_between(
        full_x, mean - std, mean + std, alpha=0.3, color="blue", label="68% Confidence"
    )
    ...
    n_obs = len(valid_xs)
    ax1.set_title(
        f"Bonni - 1D ({n_obs} observations)", fontsize=14, fontweight="bold", pad=15
    )
    ...
    ax2.plot(
        full_x, full_acq_vals, "purple", linewidth=2.5, label="Acquisition Function"
    )

    if next_point is not None:
        ax2.axvline(next_point, color="green", linestyle="--", linewidth=2, alpha=0.7)
